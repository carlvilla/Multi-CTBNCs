...
public class Dataset {
    ...
    /**
     * Registers the name of a feature in the dataset to allow the inclusion of sequences that contain it. This method
     * requires a fill state that will be used to include the variable in those sequences.
     *
     * @param nameFeatureVariable feature variable name
     */
    public void addFeatureVariable(String nameFeatureVariable) {
        this.nameFeatureVariables.add(nameFeatureVariable);
        for (Sequence sequence : getSequences()) {
            boolean isFeatureVariableAdded = sequence.addFeatureVariable(nameFeatureVariable, sequence);
            if (!isFeatureVariableAdded) {
                logger.error("The variable {} is already in the dataset", nameFeatureVariable);
                break;
            }
        }
    }
    ...
    /**
     * Add a new feature variable to the dataset given the sequences containing the transitions of the variable. It is
     * assumed that the given sequences and those of the dataset have the same length. This method ignores the time
     * variable.
     *
     * @param nameFeatureVariable name of the feature variable to add
     * @param dataset             dataset containing sequences with the new feature variable
     */
    public void addFeatureVariable(String nameFeatureVariable, Dataset dataset) {
        List<Sequence> sequencesNewFeatureVariable = dataset.getSequences();
        this.nameFeatureVariables.add(nameFeatureVariable);
        IntStream.range(0, sequencesNewFeatureVariable.size()).forEach(idxSequence -> {
            getSequences().get(idxSequence).addFeatureVariable(nameFeatureVariable,
                    sequencesNewFeatureVariable.get(idxSequence));
        });
    }
     ...
     *
     * @return multi-class dataset
     */
    public Dataset getLabelPowerset() {
        logger.info("Using label powerset to generate multi-class dataset...");
        String nameClassVariableLP = "ClassLP";
        List<String> nameClassVariables = getNameClassVariables();
        List<Sequence> newSequences = new ArrayList<>();
        for (Sequence sequence : this.sequences) {
            String classConfigurationSequence = "";
            for (String nameClassVariable : nameClassVariables) {
                classConfigurationSequence =
                        classConfigurationSequence + sequence.getValueClassVariable(nameClassVariable);
            }
            double[] timeValues = sequence.getTimeValues();
            String[][] variableValuesSequence = sequence.getVariablesValues();

            Sequence newSequence = new Sequence(this.nameTimeVariable, this.nameFeatureVariables,
                    List.of(nameClassVariableLP), timeValues, variableValuesSequence,
                    new String[]{classConfigurationSequence}, sequence.getIdxFeatureVariables());
            newSequences.add(newSequence);
        }
        return new Dataset(newSequences);
    }
     ...
     *
     * @param namesFeatureVariables names of the feature variables
     */
    public void removeFeatureVariables(List<String> namesFeatureVariables) {
        for (String nameFeatureVariable : namesFeatureVariables) {
            removeFeatureVariable(nameFeatureVariable);
        }
    }
    ...
    public void removeFeatureVariable(String nameFeatureVariable) {
        this.nameFeatureVariables.remove(nameFeatureVariable);
        for (Sequence sequence : getSequences()) {
            sequence.removeFeatureVariable(nameFeatureVariable);
        }
    }
