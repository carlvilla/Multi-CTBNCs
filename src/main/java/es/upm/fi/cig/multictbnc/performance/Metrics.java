...
public class Metrics {
    ...
    public static double macroAveraging(Prediction[] predicted, Dataset actualDataset, Metric metric) {
        ...
        for (String nameCV : nameCVs) {
            ...
            System.out.println(metric.toString() + " (macro) " + nameCV + ": " + metricResultCV);
    ...
    private static Map<String, Double> evaluateMultiDimensionalClassification(Prediction[] predicted,
        ...
        for (String nameClassVariable : actualDataset.getNameClassVariables()) {
            State[] predictedClasses =
                    Arrays.stream(predicted).map(sequence -> sequence.getPredictedClasses()).toArray(
                            State[]::new);
            State[] actualClasses = actualDataset.getStatesClassVariables();
            String[] predictedClassesCV = Arrays.stream(predictedClasses).map(
                    state -> state.getValueVariable(nameClassVariable)).toArray(String[]::new);
            String[] actualClassesCV = Arrays.stream(actualClasses).map(
                    state -> state.getValueVariable(nameClassVariable)).toArray(String[]::new);
            List<String> possibleClassesCV = actualDataset.getPossibleStatesVariable(nameClassVariable);
            Map<String, Integer> confusionMatrix = getConfusionMatrix(predictedClassesCV, actualClassesCV,
                    possibleClassesCV);


            System.out.println("-------Confusion matrix " + nameClassVariable + "-------");
    ...
    private static Map<String, Double> evaluateOneDimensionalClassification(Prediction[] predicted,
        ...
        // There is only one class variable
        String nameClassVariable = actualDataset.getNameClassVariables().get(0);
        State[] predictedClasses = Arrays.stream(predicted).map(sequence -> sequence.getPredictedClasses()).toArray(
                State[]::new);
        State[] actualClasses = actualDataset.getStatesClassVariables();
        String[] predictedClassesCV = Arrays.stream(predictedClasses).map(
    ...
    /**
     * Obtains a confusion matrix given a list of predicted values and their actual values.
     *
     * @param predicted {@code Prediction} array
     * @param actual    dataset with actual classes
     * @return confusion matrix
     */
    private static Map<String, Integer> getConfusionMatrix(String[] predicted, String[] actual,
                                                           List<String> possibleValues) {
        // Initialise confusion matrix
        Map<String, Integer> confusionMatrix = new HashMap<>();
        for (String predictedValue : possibleValues) {
            for (String actualValue : possibleValues) {
                confusionMatrix.put(predictedValue + "/" + actualValue, 0);
            }
        }
        for (int i = 0; i < predicted.length; i++)
            confusionMatrix.computeIfPresent(predicted[i] + "/" + actual[i], (k, v) -> v + 1);
