...
public class PC implements StructureLearningAlgorithm {
    ...
    public void learn(PGM<? extends Node> pgm) throws ErroneousValueException {
        ...
        if (pgm.getNumNodes() > 1) {
            ...
            // If there are undirected edges, i.e., we have a PDAG. As a DAG is necessary, undirected edges will
            // be oriented arbitrarily. However, we need to be careful not to introduce v-structures
     ...
     * while the method returns the separation sets.
     *
     * @param pgm             probabilistic graphical model
    ...
    void addParentsToNode(PGM<? extends Node> pgm, DiscreteStateNode node, List<Integer> subset) {
        for (int idxNodeSubset : subset)
            pgm.getNodeByIndex(idxNodeSubset).setChild(node);
    }

    boolean testConditionalDependence(CPTNode nodeX, CPTNode nodeY,
        ...
        // If chiSquared >= criticalValue, the null hypothesis is rejected, and variables are conditionally dependent
     ...
     */
    int getNumSamplesGivenSepSet(CPTNode nodeX, CPTNode nodeY,
                                 BNSufficientStatistics suffStatNodeXGivenNodeYSepSet) {
        int numSamples = 0;
        for (int idxStateNodeY = 0; idxStateNodeY < nodeY.getNumStates(); idxStateNodeY++) {
            nodeY.setState(idxStateNodeY);
