...
public class CTPC implements StructureLearningAlgorithm {
    ...
    /**
     * Defines a node, which is being studied as a possible parent node, and a separating set as parents of an evaluated
    ...
    protected void retrieveParametersAndSuffStatistics(PGM<? extends Node> pgm, int idxNode,
                                                       Map<String, List<Object>> cache, List<Integer> idxSepSet,
                                                       Integer... idxParents) {
        CIMNode node = (CIMNode) pgm.getNodeByIndex(idxNode);
        // Access cache to retrieve parameters and sufficient statistics
        String keyCache = computeKeyCache(pgm, idxSepSet, idxParents);
    ...
    protected boolean testNullTimeToTransitionHypForGivenSepSet(CIMNode node, CIMNode parentNode,
        ...
        for (int idxState = 0; idxState < node.getNumStates(); idxState++) {
            ...
            for (int idxStateSepSetAndParent = 0; idxStateSepSetAndParent < node.getNumStatesParents();
                 idxStateSepSetAndParent++) {
                // Perform the hypothesis test
                boolean isNullTimeToTransitionHypAccepted = testNullTimeToTransitionHyp(node, nameNodesSepSet, qxSepSet,
     ...
     */
    private String computeKeyCache(PGM<? extends Node> pgm, List<Integer> idxSepSet, Integer... idxParents) {
        // Get indexes of nodes and sort them
        List<Integer> indexesNodes = new ArrayList<>(Arrays.asList(idxParents));
        if (idxSepSet != null && idxSepSet.size() > 0)
            indexesNodes.addAll(idxSepSet);
        Collections.sort(indexesNodes);
        // Get names of the nodes
        List<String> namesNodes =
                indexesNodes.stream().map(idx -> pgm.getNodeByIndex(idx).getName()).collect(Collectors.toList());
        return String.join(",", namesNodes);
    }
    ...
    /**
     * Evaluate the null state-to-state transition hypothesis for a given state of the feature, class variable, and
     * features in the separating set.
     *
     * @param node                    evaluated node
     * @param nameNodesSepSet         names of the nodes in the separating set
     * @param suffStatSepSet          sufficient statistics of the node given the separating set
     ...
     * @param idxStateSepSetAndParent index of the combined state of the class variable and the separating set
     * @return {@code true} if the null hypothesis is not rejected, {@code false} otherwise
     */
    ...
    /**
     * Evaluate the null time to transition hypothesis for a given state of the node and its parents.
     *
     * @param node                    evaluated node
     * @param nameNodesSepSet         names of the nodes in the separating set
     * @param qxSepSet                parameters of the node given the separating set
