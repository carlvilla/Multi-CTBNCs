...
public class BNHillClimbing implements HillClimbingImplementation {
	...
	public HillClimbingSolution findStructure(PGM<? extends Node> pgm) {
		...
		this.bn = (BN<? extends Node>) pgm;
		boolean[][] currentStructure = this.bn.getAdjacencyMatrix().clone();
		...
		do {
			...
			findBestNeighbor(currentStructure, scores, adjacencyMatrices, "addition");
			// Structures generated by deletion
			findBestNeighbor(currentStructure, scores, adjacencyMatrices, "deletion");
			// Structures generated by reversal
			findBestNeighbor(currentStructure, scores, adjacencyMatrices, "reversal");
		...
		for (int i = 0; i < numNodes; i++) {
			for (int j = 0; j < numNodes; j++)
				if (i != j) {
					...
					if (this.bn.isStructureLegal(tempAdjacencyMatrix)) {
						logger.trace("Studying new {} structure: {}", this.bn.getType(), tempAdjacencyMatrix);
	...
	private double computeScore(boolean[][] adjacencyMatrix) {
		...
		this.bn.setStructureModifiedNodes(adjacencyMatrix);
		return this.scoreFunction.compute(this.bn);
	}
