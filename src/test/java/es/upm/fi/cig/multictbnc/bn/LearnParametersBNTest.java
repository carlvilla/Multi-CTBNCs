package es.upm.fi.cig.multictbnc.bn;
...
/**
 * Tests the estimation of sufficient statistics and parameters of a BN. Maximum likelihood estimation test is executed
 * first, so the sufficient statistics are computed before their test starts.
 *
...
class LearnParametersBNTest {
	...
	public static void setUp() {
		...
		// C2 and C3 are the parents of C1, and C3 is the parent of C2
	...
	public void testMLEParameters() {
		...
		nodeC2.setState("b");
		nodeC3.setState("b");
		int idxStateParents = nodeC1.getIdxStateParents();
	...
	/**
	 * Check the parameters of a BN obtained by Bayesian estimation. The imaginary counts are all established to 0, so
	 * the result should be the same as with maximum likelihood estimation.
	 */
	@Test
	...
	/**
	 * Check the parameters of a BN obtained by Bayesian estimation. All the imaginary counts are more than 0.
	 */
	@Test
	public void testBE_ImaginaryCountsOn() {
		...
		bnParameterLearning.learn(nodes, dataset);
		// Sufficient statistics
		testBESufficientStatistic(NxPrior);
		// Parameters
		testBEParameters(NxPrior);
	}

	/**
	 * Check the estimation of the parameters of a given BN and dataset when using Bayesian estimation.
	 *
	 * @param NxPrior hyperparameter
	 */
	public void testBEParameters(double NxPrior) {
		// Retrieve nodes
		CPTNode nodeC1 = nodes.get(0);
		CPTNode nodeC2 = nodes.get(1);
		CPTNode nodeC3 = nodes.get(2);
		...
		double nxHPC1 = NxPrior / (nodeC1.getNumStatesParents() * nodeC1.getNumStates());
		double nxHPC2 = NxPrior / (nodeC2.getNumStatesParents() * nodeC2.getNumStates());
		double nxHPC3 = NxPrior / (nodeC3.getNumStatesParents() * nodeC3.getNumStates());
		...
		nodeC3.setState("b");
		int idxStateParents = nodeC1.getIdxStateParents();
		assertEquals((3 + nxHPC1) / (6 + nxHPC2), nodeC1.getCP(idxStateParents, idxFromStateNode), 0.001);
		...
		nodeC3.setState("b");
		idxStateParents = nodeC2.getIdxStateParents();
		assertEquals((6 + nxHPC2) / (6 + nxHPC3), nodeC2.getCP(idxStateParents, idxFromStateNode), 0.001);

		idxFromStateNode = nodeC2.setState("a");
		nodeC3.setState("a");
		idxStateParents = nodeC2.getIdxStateParents();
		assertEquals((1 + nxHPC2) / (1 + nxHPC3), nodeC2.getCP(idxStateParents, idxFromStateNode), 0.001);

		// Check parameters C3
		idxFromStateNode = nodeC3.setState("a");
		idxStateParents = nodeC3.getIdxStateParents();
		assertEquals((1 + nxHPC3) / (dataset.getNumDataPoints() + NxPrior),
				nodeC3.getCP(idxStateParents, idxFromStateNode), 0.001);

		idxFromStateNode = nodeC3.setState("b");
		idxStateParents = nodeC3.getIdxStateParents();
		assertEquals((6 + nxHPC3) / (dataset.getNumDataPoints() + NxPrior),
				nodeC3.getCP(idxStateParents, idxFromStateNode), 0.001);
	...
	/**
	 * Check the computation of the sufficient statistics of a given BN and dataset when using Bayesian estimation.
	 * They
	 * should include the defined hyperparameters.
	 *
	 * @param NxPrior hyperparameter
	 */
	public void testBESufficientStatistic(double NxPrior) {
		// Retrieve nodes
		CPTNode nodeC1 = nodes.get(0);
		CPTNode nodeC2 = nodes.get(1);
		CPTNode nodeC3 = nodes.get(2);
		...
		Integer idxStateNode = nodeC1.getIdxState();
		int idxStateParents = nodeC1.getIdxStateParents();
		assertEquals(3 + nxHPC1, nxC1[idxStateParents][idxStateNode]);
		...
		idxStateNode = nodeC1.getIdxState();
		idxStateParents = nodeC1.getIdxStateParents();
		assertEquals(0 + nxHPC1, nxC1[idxStateParents][idxStateNode]);
		...
		idxStateNode = nodeC1.getIdxState();
		idxStateParents = nodeC1.getIdxStateParents();
		assertEquals(2 + nxHPC1, nxC1[idxStateParents][idxStateNode]);
		...
		idxStateNode = nodeC2.getIdxState();
		idxStateParents = nodeC2.getIdxStateParents();
		assertEquals(6 + nxHPC2, nxC2[idxStateParents][idxStateNode]);

		nodeC2.setState("a");
		nodeC3.setState("a");
		idxStateNode = nodeC2.getIdxState();
		idxStateParents = nodeC2.getIdxStateParents();
		assertEquals(1 + nxHPC2, nxC2[idxStateParents][idxStateNode]);

		// Check sufficient statistics C3
		nodeC3.setState("a");
		idxStateParents = nodeC3.getIdxStateParents();
		idxStateNode = nodeC3.getIdxState();
		assertEquals(1 + nxHPC3, nxC3[idxStateParents][idxStateNode]);

		nodeC3.setState("b");
		idxStateParents = nodeC3.getIdxStateParents();
		idxStateNode = nodeC3.getIdxState();
		assertEquals(6 + nxHPC3, nxC3[idxStateParents][idxStateNode]);
