...
public class LearnSubmodels {
	 ...
	 * algorithms. The dataset is formed by five discrete-state variables, four feature variables (X1, X2, X3 and X4)
	 * and two class variables (C1 and C2). The data were sampled from a Multi-CTBNC with the following structure:
	 * X4 <-
	 * X3 -> X2 <-> X1 <- C1 -> C2 -> X2.
	 *
	...
	public void learnEmptyDigraphMultiCTBNC() throws ErroneousValueException {
		// Define model
		MultiCTBNC multiCTBNC = ClassifierFactory.getMultiCTBNC("Empty-digraph Multi-CTBNC", bnLearningAlgs,
				ctbnLearningAlgs);
		// Learn model
		multiCTBNC.learn(dataset);
	...
	public void learnEmptyMaxKMultiCTBNC() throws ErroneousValueException {
		// Define model
		Map<String, String> hyperparameters = Map.of("maxK", "1");
		MultiCTBNC multiCTBNC = ClassifierFactory.getMultiCTBNC("Empty-maxK Multi-CTBNC", bnLearningAlgs,
				ctbnLearningAlgs, hyperparameters);
		// Learn model
		multiCTBNC.learn(dataset);
