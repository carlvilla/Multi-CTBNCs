...
/**
 * Tests to evaluate the learning of multidimensional continuous time Bayesian networks.
 * 
...
public class LearnMCTBNC {
	 ...
	 * state of C2. If C2='a', the transitions of F2 are always 'a'->'b'->'c'->'a',
	 * if C2='b', the transitions of F2 are always 'a'->'c'->'b'->'a', if C2='c', if
	 * C2='c', the transitions of F2 are always 'a'->'c'->'a', while if C2='d', the
	 * transitions of F2 are always 'b'->'c'->'b'.
	 * 
	...
	public static void setUp() {
		...
		for (int i = 0; i < numObservations; i++) {
			...
			dataSequence4
					.add(new String[] { String.valueOf(i * 0.1), valuesF1[i], valuesF2[i], valuesF3[i], "2", "c" });
		}
	...
	public void learnMultidimensionalContinuousTimeBayesianNetwork() {
		MCTBNC<CPTNode, CIMNode> mctbnc = new MCTBNC<CPTNode, CIMNode>(dataset, ctbnParameterLearningAlgorithm, ctbnStructureLearningAlgorithm,
				bnParameterLearningAlgorithm, bnStructureLearningAlgorithm, CPTNode.class, CIMNode.class);
		mctbnc.learn();
